 ...
 *  -@Component("View 의 이름") view의 이름을 지정한다. 
 *  -servlet-context.xml 에 BeanNameViewResolver 설정하기 (최우선순위로!  name="order" value="0")
 *  
 */

@Component("fileDownView")//이 뷰의 이름은 fileDownView
public class FileDownView extends AbstractView{ //AbstractView 를 상속받앗기때문에 view의 역할을 할수 있다.
	//여기로 포워드 이동한거나 다름 없다. 애도 빈이 되고 빈의 이름은 fileDownView 이다. 
	@Override
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		//모델에 전달된거는 Map에 전달된다. 키와 밸류값으로...
		//ModelAndView 객체에 .addObject() 해서 담은 객체는 
		//인자로 전달된 Map 객체에 들어 있다.
		//ModelAndView 객체에 담아 주었던 dto 를 얻어온다. 
		FileDto dto=(FileDto)model.get("dto"); //Map에 담겨있기때문에 형변환해주며 가져온다. 
